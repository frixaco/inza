...
const defaultDeckPath = "parser/Kaishi-1.5k-v2.4.apkg";
const defaultOutputDir = "kaishi-open-deck";
...
const mainAnswerFields = new Set(["Word Meaning", "Sentence Meaning"]);
const overlineMark = "\uffe3";
const dropMark = "\uff3c";

function usage(): never {
  console.error("Usage: bun tools/convert-kaishi-to-open-deck.ts [apkg] [output-dir] [--force]");
  process.exit(2);
}
...
function createTextRun(text: string, attrs: RunAttrs, extra?: { above?: string; below?: string }): InlineRun {
...
function addAdornment(runs: InlineRun[], start: number, end: number, key: "above" | "below", value: string): void {
    ...
    if (token.startsWith("<")) {
      ...
      if (style.includes("border-top") && markerTarget) {
        ...
        if (style.includes("border-right")) {
          addAdornment(runs, Math.max(markerTarget.runStart, runs.length - 1), runs.length, "below", dropMark);
        }
      }
...
function blocksForFields(fields: Record<string, string>, names: readonly string[]): DraftContentBlock[] {
...
function buildOpenNote(note: Note, index: number, copiedMedia: Map<string, string>): OpenNote {
  ...
  return {
    ...
    provenance: {
      importer: "tools/convert-kaishi-to-open-deck.ts",
...
async function main(): Promise<void> {
  ...
  await writeYaml(path.join(outputDir, "deck.yaml"), {
    ...
    description: "Japanese vocabulary cards migrated from the Kaishi Anki deck into Open Deck content blocks.",
    language: "ja",
  });

  const copiedMedia = await copyReferencedMedia(parsed.notes, path.join(importWorkDir, "media"), outputDir);
