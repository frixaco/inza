...
const sharedNoteFields = new Set(["id", "type", "deck", "tags", "language", "answer_mode", "media", "references", "provenance"]);
...
function validateStringList(value: unknown, issuePath: string, fieldName: string, issues: Issue[]): void {
  if (value === undefined) {
    return;
  }
  ...
  value.forEach((item, index) => {
    if (!isNonEmptyString(item)) {
      issues.push({ path: `${issuePath}[${index}]`, message: `${fieldName} item must be a non-empty string` });
    }
  });
}
...
function validateContent(deckRoot: string, value: unknown, issuePath: string, issues: Issue[]): void {
  ...
  value.forEach((block, index) => {
    ...
    if (block.text !== undefined && block.runs !== undefined) {
      issues.push({ path: blockPath, message: "content block must not include both text and runs" });
    }
    ...
    if (block.language !== undefined && !isNonEmptyString(block.language)) {
      issues.push({ path: blockPath, message: "content block language must be a non-empty string" });
    }
    ...
    if (run.marks !== undefined) {
      ...
      run.marks.forEach((mark, markIndex) => {
        if (typeof mark !== "string" || !inlineMarks.has(mark)) {
          ...
          issues.push({ path: `${runPath}.marks[${markIndex}]`, message: `duplicate run mark: ${mark}` });
        } else {
          seenMarks.add(mark);
        }
      });
...
function validateMedia(deckRoot: string, media: unknown, issuePath: string, issues: Issue[]): void {
  ...
  if (media.length === 0) {
    issues.push({ path: issuePath, message: "media must not be empty" });
    return;
  }
...
function validateReferences(value: unknown, issuePath: string, issues: Issue[]): void {
  ...
  value.forEach((reference, index) => {
    ...
    if (reference.title === undefined && reference.url === undefined && reference.locator === undefined) {
      issues.push({ path: referencePath, message: "reference must include title, url, or locator" });
    }
  });
}

function validateSharedNoteFields(note: Record<string, unknown>, issuePath: string, issues: Issue[]): void {
  ...
  if (note.answer_mode !== undefined && (typeof note.answer_mode !== "string" || !answerModes.has(note.answer_mode))) {
    issues.push({ path: `${issuePath}.answer_mode`, message: "answer_mode must be one of reveal, typed" });
  }
...
function validateCloze(deckRoot: string, note: Record<string, unknown>, issuePath: string, issues: Issue[]): void {
...
function validateOcclusion(deckRoot: string, note: Record<string, unknown>, issuePath: string, issues: Issue[]): void {
  ...
  if (!isRecord(note.image)) {
    ...
    if (note.image.alt !== undefined && typeof note.image.alt !== "string") {
      issues.push({ path: `${issuePath}.image.alt`, message: "occlusion image alt must be a string" });
    }
  ...
  note.masks.forEach((mask, index) => {
    ...
    if (!isNonEmptyString(mask.id)) {
      issues.push({ path: `${maskPath}.id`, message: "occlusion mask id must be a non-empty string" });
    ...
    if (mask.answer !== undefined && typeof mask.answer !== "string") {
      issues.push({ path: `${maskPath}.answer`, message: "occlusion mask answer must be a string" });
    }
    ...
    if (mask.group !== undefined && !isNonEmptyString(mask.group)) {
      issues.push({ path: `${maskPath}.group`, message: "occlusion mask group must be a non-empty string" });
    }
    ...
    if (typeof mask.shape.kind !== "string" || !maskShapes.has(mask.shape.kind)) {
      issues.push({ path: `${maskPath}.shape.kind`, message: "occlusion shape kind must be one of rect, ellipse, polygon" });
    }

    if (mask.shape.kind === "polygon") {
      if (!Array.isArray(mask.shape.points) || mask.shape.points.length < 3) {
        issues.push({ path: `${maskPath}.shape.points`, message: "polygon shape requires at least three points" });
      } else {
        mask.shape.points.forEach((point, pointIndex) => {
          if (!Array.isArray(point) || point.length !== 2) {
            issues.push({ path: `${maskPath}.shape.points[${pointIndex}]`, message: "polygon point must be [x, y]" });
            return;
          }
  ...
  if (!existsSync(manifestPath)) {
    ...
    if (!isRecord(manifest)) {
      ...
      if (manifest.description !== undefined && !isNonEmptyString(manifest.description)) {
        issues.push({ path: `${manifestPath}.description`, message: "description must be a non-empty string" });
      }
      if (manifest.language !== undefined && !isNonEmptyString(manifest.language)) {
        issues.push({ path: `${manifestPath}.language`, message: "language must be a non-empty string" });
      }
      if (manifest.license !== undefined && !isNonEmptyString(manifest.license)) {
        issues.push({ path: `${manifestPath}.license`, message: "license must be a non-empty string" });
      }
    }
  }
