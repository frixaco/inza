...
private enum AppPalette {
    static let canvas = adaptive(light: ns(0.965, 0.956, 0.936), dark: .windowBackgroundColor)
    static let sidebar = adaptive(light: ns(0.928, 0.918, 0.894), dark: .underPageBackgroundColor)
    static let panel = adaptive(light: ns(0.946, 0.936, 0.912), dark: .controlBackgroundColor)
    static let panelSubtle = adaptive(light: ns(0.928, 0.918, 0.895), dark: .controlBackgroundColor)
    static let separator = adaptive(light: ns(0.760, 0.735, 0.690), dark: .separatorColor)
    static let keyCap = adaptive(light: ns(0.890, 0.878, 0.850), dark: .controlBackgroundColor)

    static let canvasNS = adaptiveNS(light: ns(0.965, 0.956, 0.936), dark: .windowBackgroundColor)

    private static func ns(_ red: CGFloat, _ green: CGFloat, _ blue: CGFloat) -> NSColor {
...
private struct WindowChromeConfigurator: NSViewRepresentable {
    ...
    private static func configure(window: NSWindow?) {
        ...
        window.backgroundColor = AppPalette.canvasNS
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            NoteInspector(card: selectedCard)
                .frame(width: 326)
                .background(AppPalette.panelSubtle)
        }
    }
}
    ...
    private var promptPanel: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            TextEditor(text: $prompt)
                ...
                .frame(minHeight: 178)
                .padding(12)
                .background(AppPalette.panelSubtle, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
    ...
    var body: some View {
        ViewThatFits(in: .horizontal) {
            HStack(spacing: 0) {
                ...
                Text("FSRS \(fsrsTarget)%")
                    ...
                    .padding(.vertical, 4)
                    .background(AppPalette.keyCap, in: Capsule())
            }
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke(AppPalette.separator.opacity(0.42), lineWidth: 1)
        }
    }
...
private struct MetricStrip: View {
    ...
    var body: some View {
                            ...
                            MetricCell(metric: metric)
                                .overlay(alignment: .trailing) {
                                    if metric.id != metrics.last?.id {
                                        Rectangle()
                                            .fill(AppPalette.separator.opacity(0.55))
                                            .frame(width: 1)
                                            .padding(.vertical, 14)
                                    }
                                }
        ...
        .background(AppPalette.panel, in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        .overlay {
...
private struct BrowserTag: View {
    ...
    var body: some View {
        Text(title)
            ...
            .padding(.vertical, 3)
            .background(AppPalette.keyCap, in: RoundedRectangle(cornerRadius: 5, style: .continuous))
    }
}
...
private struct SourceChip: View {
    ...
    var body: some View {
        Label(title, systemImage: symbol)
            ...
            .padding(.horizontal, 10)
            .padding(.vertical, 7)
            .background(AppPalette.panelSubtle, in: RoundedRectangle(cornerRadius: 8, style: .continuous))
    }
}
        ...
        Text(key)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 5, style: .continuous)
                    .stroke(AppPalette.separator.opacity(0.45), lineWidth: 1)
            }
    }
}
