...
async function loadCollection(zip: JSZip): Promise<CollectionSource> {
  const entryName = ["collection.anki21b", "collection.anki21", "collection.anki2"].find((name) =>
    zip.file(name),
  );
...
async function loadMediaManifest(zip: JSZip): Promise<MediaEntry[]> {
  ...
  return entries.map((field, index) => {
    ...
    const hashField = nestedFields.find(
      (nestedField) =>
        nestedField.fieldNumber === 3 &&
        nestedField.wireType === 2 &&
        nestedField.value instanceof Uint8Array,
    );
...
function loadModernMetadata(db: Database): {
  ...
  const deckRows = db.query("SELECT id, name FROM decks ORDER BY id").all() as Array<{
    id: number;
    name: string;
  }>;
  const notetypeRows = db.query("SELECT id, name FROM notetypes ORDER BY id").all() as Array<{
    id: number;
    name: string;
  }>;
  const fieldRows = db
  ...
  for (const notetype of notetypeRows) {
    const templates = (templatesByNoteType.get(notetype.id) || []).sort(
...
export async function parseApkg(
  ...
  try {
    const col = db
      .query("SELECT ver, crt, models, decks FROM col LIMIT 1")
      .get() as CollectionRow | null;
    if (!col) {
    ...
    for (const cardRow of cardRows) {
      ...
      const back = template
        ? renderTemplate(template.afmt, fields, cardRow.ord, {
            frontSide: front,
            revealCloze: true,
          })
    ...
    const result: ParseResult = {
      ...
      noteTypes: Object.fromEntries(
        Array.from(noteTypes.values()).map((noteType) => [
          String(noteType.output.id),
          noteType.output,
        ]),
      ),
      media,
